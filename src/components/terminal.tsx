...
export default function Terminal({ 
  ...
  const handleCommand = async () => {
    ...
    if (executable) {
        ...
        let shouldProcess = true; // For commands that don't need auth check first.
        let targetPC = getCurrentPc(); // Get current PC state.

        if (cmdName !== 'scan' && cmdName !== 'probe') { // These can be run without full auth
            if(!checkAuth()) {
                shouldProcess = false;
            }
        }
